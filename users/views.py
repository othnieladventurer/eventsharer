...
class CustomSignupView(SignupView):
    ...
    success_url = reverse_lazy('eventadm:admin')

    def form_valid(self, form):
        response = super().form_valid(form)

        user = self.user  # the new signed-up user

        # Create trial only if it doesn't already exist
        if not hasattr(user, 'trial'):
            Trial.objects.create(
                user=user,
                start_date=timezone.now(),
                end_date=timezone.now() + timedelta(days=7)
            )
            print(f"[DEBUG] Trial created for {user.email}")

        return response

    def form_invalid(self, form):
        print("[DEBUG] Signup form is invalid")
        print(form.errors)
...
class CustomPasswordResetView(PasswordResetView):
    template_name = 'users/account/password_reset.html'  # Your custom template
